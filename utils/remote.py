...
def remote_login(client, host, port, username, password, prompt,
    ...
    :param authentication: The PreferredAuthentications. (password/publickey).
...
def wait_for_login(client, host, port, username, password, prompt,
                   ...
                   interface=None, authentication="publickey", options=''):
    """
...
def scp_to_remote(host, port, username, password, local_path, remote_path,
    ...
    :param interface: The interface the neighbours attach to (only use when using ipv6
            linklocal address.)
    :param authentication: The authentication method (password/publickey)
...
def scp_between_remotes(src, dst, port, s_passwd, d_passwd, s_name, d_name,
    ...
    command = ("scp -v -o UserKnownHostsFile=/dev/null "
               "-o StrictHostKeyChecking=no "
               "-o PreferredAuthentications=%s -r %s -P %s"
               " %s@\[%s\]:%s %s@\[%s\]:%s" %
               (authentication, limit, port, s_name, src, s_path, d_name, dst, d_path))
    password_list = []
